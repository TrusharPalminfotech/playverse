...
2. **Steps Execution**:
   ...
   - **SonarCloud Scan**: Invokes the pinned stable `sonarsource/sonarqube-scan-action@v6` scanner, sending code statistics and test coverage to SonarCloud. With `sonar.qualitygate.wait=true` configured, the scanner polls SonarCloud directly and will fail the build if the Quality Gate fails.
